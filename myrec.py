...
# Define a global frame buffer
max_buffer_size = 10 if device == torch.device("cpu") else 5
...
def buffer_filler(cap):
    """
    Continuously reads frames from the video source and fills the buffer.
    """
    while True:
        ret, frame = cap.read()
        if not ret:
            print("Failed to read frame from source. Retrying...")
            time.sleep(0.1)  # Wait before retrying
            continue

        if frame_buffer.full():
            frame_buffer.get()  # Discard the oldest frame to make space
...
def tracking(detector, args):
    """
    Face tracking in a separate thread with buffer logic.
...
def main():
    ...
    thread_buffer = threading.Thread(target=buffer_filler, args=(cap,), daemon=True)
    thread_buffer.start()
